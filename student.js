...
window.goToStudent = async () => {
  ...
  if (!studentPrefs?.onboardingDone) {
    ...
    if (!hasStudyBlocks) { await generateStudyBlocks(); await saveStudentData(); }
...
async function generateStudyBlocks() {
  if (!studentPrefs) return;
  for (const dayKey of DAYS_EN) {
    if (!studentSchedule[dayKey]) studentSchedule[dayKey] = [];
    const oldStudy = studentSchedule[dayKey].filter(b => b.type === 'study');
    studentSchedule[dayKey] = studentSchedule[dayKey].filter(b => b.type !== 'study');
    const fresh = getAutoStudyBlocks(dayKey);

    // Même id (même créneau) qu'avant -> on récupère le lien taskId existant
    for (const block of fresh) {
      const match = oldStudy.find(b => b.id === block.id);
      if (match?.taskId) block.taskId = match.taskId;
    }
    // Anciens blocs dont le créneau a disparu -> nettoie la tâche liée orpheline
    for (const old of oldStudy) {
      const stillExists = fresh.some(b => b.id === old.id);
      if (!stillExists && old.taskId) await _deleteLinkedTaskForBlock(old);
    }
...
window.saveCourseBlock = async () => {
  ...
  const oldBlock = editingCourseId ? (studentSchedule[editingCourseDay]||[]).find(c=>c.id===editingCourseId) : null;
  ...
  if (oldBlock?.taskId) block.taskId = oldBlock.taskId; // préserve le lien tâche existant
  ...
  if (!editingCourseId && selectedCourseType==='revision' && !editingIsAutoStudy) {
    // Nouvelle révision manuelle -> crée la tâche liée (si le toggle sync est actif)
    await _createLinkedTask(block, editingCourseDay);
  } else if (editingCourseId && (block.type==='revision' || block.type==='study') && block.taskId) {
    // Bloc revision/study existant et déjà lié -> répercute la modif sur la tâche
    await _syncTaskFromBlock(block, editingCourseDay);
  }
...
// ── SYNC RÉVISIONS ↔ TÂCHES ──────────────────────────────────────────────────
// Convertit une date (YYYY-MM-DD) en dayKey (DAYS_EN), pour la sync tâche -> bloc
function _dayKeyFromDateStr(dateStr) {
  const d  = new Date(dateStr + 'T00:00:00');
  const js = d.getDay(); // 0=dimanche..6=samedi
  return DAYS_EN[js === 0 ? 6 : js - 1];
}

// Prochaine date (YYYY-MM-DD) correspondant à un dayKey, à partir d'aujourd'hui
function _nextDateForDay(dayKey) {
  const dayIdx   = DAYS_EN.indexOf(dayKey);
  const now      = new Date();
  const jsDayIdx = dayIdx === 6 ? 0 : dayIdx + 1;
  let diff = jsDayIdx - now.getDay();
  if (diff < 0) diff += 7;
  const taskDate = new Date(now); taskDate.setDate(now.getDate() + diff);
  return window.fmtDate ? window.fmtDate(taskDate) : taskDate.toISOString().slice(0,10);
}

// Crée (si le toggle est activé) une tâche liée à un bloc revision/study, et pose le lien block.taskId <-> task.revisionBlockId
async function _createLinkedTask(block, dayKey) {
  if (studentPrefs?.tasksSyncEnabled === false) return null;
  const dateStr = _nextDateForDay(dayKey);
  const ts      = Date.now();
  const isStudy = block.type === 'study';
  const prefix  = isStudy ? 'study' : 'rev';
  const newId   = `${prefix}-${dayKey}-${ts}-${Math.random().toString(36).slice(2,5)}`;
  const subj    = studentSubjects.find(s=>s.id===block.subjectId);
  const newTask = {
    id:newId, uid:currentUser.uid,
    title: isStudy ? block.name : `Révision — ${subj?.name||block.name}`,
    content: isStudy ? `Session d'étude : ${block.start}–${block.end}` : `Session de révision : ${block.start}–${block.end}`,
    date:dateStr, time:block.start, status:'pending',
    color: isStudy ? '#00f7ff' : (subj?.color||'#7c3aed'), alarm:true,
    studentStudy:true, studentDay:dayKey, revisionBlockId:block.id,
...
// Répercute une modification de bloc (revision/study) sur la tâche liée
async function _syncTaskFromBlock(block, dayKey) {
  if (!block.taskId) return;
  const t = tasks.find(x => x.id === block.taskId && x.status !== 'deleted');
  if (!t) { block.taskId = null; return; }
  const isStudy = block.type === 'study';
  const subj    = studentSubjects.find(s=>s.id===block.subjectId);
  const updated = {
    ...t,
    ...
    content: isStudy ? `Session d'étude : ${block.start}–${block.end}` : `Session de révision : ${block.start}–${block.end}`,
    time:    block.start,
    color:   isStudy ? '#00f7ff' : (subj?.color||t.color),
    updatedAt: Date.now()
  };
  await idbPut('tasks', updated);
  tasks = tasks.map(x => x.id === block.taskId ? updated : x);
  await queuePush('update','tasks',block.taskId,{title:updated.title,content:updated.content,time:updated.time,color:updated.color});
  renderTaskList?.();
}

// Supprime (soft) la tâche liée à un bloc, si elle existe encore
async function _deleteLinkedTaskForBlock(block) {
  if (!block?.taskId) return;
  const t = tasks.find(x => x.id === block.taskId);
  if (t && t.status !== 'deleted') {
    const now = Date.now();
    await idbPut('tasks', { ...t, status:'deleted', deletedAt:now, updatedAt:now });
    tasks = tasks.map(x => x.id === block.taskId ? { ...x, status:'deleted', deletedAt:now } : x);
    await queuePush('delete','tasks',block.taskId,{});
  }
  block.taskId = null;
}

// Appelé par tasks.js quand une tâche liée à une révision/session est supprimée : supprime le bloc côté Student
window.unlinkStudentBlockForTask = async (revisionBlockId, dayKey) => {
  if (!revisionBlockId || !dayKey || !studentSchedule[dayKey]) return;
  const before = studentSchedule[dayKey].length;
  studentSchedule[dayKey] = studentSchedule[dayKey].filter(b => b.id !== revisionBlockId);
  if (studentSchedule[dayKey].length !== before) {
    await saveStudentData();
    if (document.getElementById('screen-student')?.classList.contains('active')) renderStudentSchedule();
  }
};

// Appelé par tasks.js quand une tâche liée est modifiée : répercute horaire/jour sur le bloc côté Student
window.syncStudentBlockFromTask = async (task) => {
  if (!task?.revisionBlockId || !task.studentDay) return;
  const oldDayKey = task.studentDay;
  const newDayKey = task.date ? _dayKeyFromDateStr(task.date) : oldDayKey;
  const list = studentSchedule[oldDayKey] || [];
  const idx  = list.findIndex(b => b.id === task.revisionBlockId);
  if (idx === -1) return;
  const block = list[idx];

  if (task.time) {
    const durMin = block.end && block.start ? (timeToAbsMin(block.end) - timeToAbsMin(block.start)) : 60;
    block.start = task.time;
    block.end   = absMinToTime(timeToAbsMin(task.time) + (task.hours ? Math.round(task.hours*60) : durMin));
  }

  if (newDayKey !== oldDayKey) {
    list.splice(idx, 1);
    if (!studentSchedule[newDayKey]) studentSchedule[newDayKey] = [];
    studentSchedule[newDayKey].push(block);
    task.studentDay = newDayKey;
    await queuePush('update','tasks',task.id,{studentDay:newDayKey});
  }
...
// Active/désactive la sync globale révisions -> tâches (toggle paramètres student)
window.toggleTasksSyncPref = async () => {
  const currentlyEnabled = studentPrefs.tasksSyncEnabled !== false;
  await window.setTasksSyncEnabled(!currentlyEnabled);
};

window.setTasksSyncEnabled = async (enabled) => {
  studentPrefs.tasksSyncEnabled = enabled;

  if (!enabled) {
    // Désactivation : supprime (soft) toutes les tâches liées à une révision/session, garde les blocs
    for (const dayKey of DAYS_EN) {
      for (const block of (studentSchedule[dayKey]||[])) {
        if ((block.type === 'revision' || block.type === 'study') && block.taskId) {
          await _deleteLinkedTaskForBlock(block);
        }
      }
    }
  } else {
    // Réactivation : recrée une tâche pour chaque bloc revision/study qui n'en a pas (couvre aussi les blocs créés avant)
    for (const dayKey of DAYS_EN) {
      for (const block of (studentSchedule[dayKey]||[])) {
        if (block.type === 'revision' || block.type === 'study') {
          const linkedTaskAlive = block.taskId && tasks.some(t => t.id === block.taskId && t.status !== 'deleted');
          if (!linkedTaskAlive) await _createLinkedTask(block, dayKey);
        }
      }
    }
  }
...
window.deleteCourseBlock = async () => {
  ...
  const block = (studentSchedule[editingCourseDay]||[]).find(c=>c.id===editingCourseId);
  if (block && (block.type==='revision' || block.type==='study') && block.taskId) {
    await _deleteLinkedTaskForBlock(block);
  }
  ...
  el.innerHTML = `<div style="display:flex;flex-direction:column;gap:16px;">
    ...
    <div class="card" style="padding:16px;">
      <div class="toggle-row" style="margin-bottom:0;">
        <span class="toggle-label"><i class="fa-solid fa-list-check"></i> Révisions comme tâches</span>
        <div class="toggle ${studentPrefs.tasksSyncEnabled!==false?'on':''}" id="tasks-sync-toggle" onclick="toggleTasksSyncPref()"></div>
      </div>
      <div style="font-size:11px;color:var(--text3);margin-top:6px;">
        Fait apparaître tes révisions (manuelles et sessions auto) dans le module Tâches. Une modif ou suppression d'un côté se répercute de l'autre.
      </div>
    </div>
    ...
    <button class="btn-secondary" style="padding:12px;border-radius:14px;font-size:13px;"
      onclick="if(confirm('Regénérer le programme de révision ? Les sessions auto (modifiées ou supprimées) seront recalculées.')){generateStudyBlocks().then(()=>{saveStudentData();closeStudentSettings();renderStudentSchedule();});}">
