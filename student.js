...
window.goToStudent = async () => {
  ...
  if (!studentPrefs?.onboardingDone) startOnboarding();
  else renderStudentSchedule();
};
...
function renderStudentSchedule() {
  ...
  DAYS_EN.forEach((dayKey, idx) => {
    ...
    const userBlocks = (studentSchedule[dayKey] || []);
    const autoBlocks = getAutoStudyBlocks(dayKey);
    const allBlocks  = [...userBlocks, ...autoBlocks];
...
window.generateStudyPlan = async () => {
  ...
  for (let i=0; i<7; i++) {
    ...
    const auto    = getAutoStudyBlocks(dayKey);
...
window.saveCourseBlock = async () => {
  ...
  const block = {
    ...
    type: selectedCourseType, name, start, end, subjectId,
  ...
  // Si c'est une révision, créer automatiquement une tâche
  if (selectedCourseType==='revision') {
